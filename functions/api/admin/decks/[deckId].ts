...
async function get(context: FunctionContext<CloudEnv>, deckId: string) {
  ...
  return json({ deck: { id: deck.id, displayName: deck.display_name, publishedAt: deck.published_at, cards: (await rows(context, deckId)).map(card) } });
}
async function update(context: FunctionContext<CloudEnv>, deckId: string) {
  ...
  const changed = cards.filter((card) => { const old = before.get(card.sourceCardId); return old && (old.front !== card.front || old.back !== card.back || old.reading !== (card.reading ?? null) || old.furigana !== (card.furigana ?? null) || old.example !== (card.exampleSentence ?? null) || old.example_translation !== (card.exampleSentenceTranslation ?? null) || old.example_furigana !== (card.exampleSentenceFurigana ?? null)); }).length;
